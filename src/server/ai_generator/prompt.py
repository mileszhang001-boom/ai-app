...
TEMPLATES = {
    "anniversary": {
        ...
        "birthday": {
            "id": "anniversary_birthday",
            "name": "生日倒计时",
            "mode": "countdown",
            "description": "期待TA的生日",
            "style_presets": ["soft-purple", "sweet-pink", "vibrant-orange", "minimal-dark"]
        }
    },
    "news": {
        "news": {
    ...
    "mediahub": {
        "mediahub": {
            "id": "mediahub_mediahub",
            "name": "三方媒体中心",
            "description": "聚合三方App（QQ音乐/小宇宙/B站/喜马拉雅）的播放内容，支持单App全屏或双App分屏",
            "style_presets": ["dynamic"]
        }
    }
}
...
1. anniversary（纪念日）
   ...
   - theme: birthday — 生日倒计时（倒数，距离TA的生日还有X天）。关键词：生日、寿星、蛋糕、birthday、庆生、生日快乐
2. news（每日新闻）
...
7. mediahub（三方媒体中心）
   - theme: mediahub — 聚合三方App播放内容。关键词：三方、播放、QQ音乐、小宇宙、B站、喜马拉雅、播客、视频、有声书、媒体中心

重要日期计算规则：
- 如果用户说"在一起N天"或"第N天"，用今天日期减去N天计算 start_date。公式: start_date = 今天 - N天。例：今天2026-03-25，"在一起1000天" → start_date = 2023-06-29
- 如果用户说"距离XX还有N天"，用今天日期加N天计算 target_date。公式: target_date = 今天 + N天
- 必须精确计算，不能近似

输出格式（严格遵循）：
{
  "template_id": "对应的模板ID（如 anniversary_love）",
  "component_type": "anniversary | news | alarm | weather | music | calendar | mediahub",
  "theme": "love | baby | holiday | news | clock | weather | music | calendar | mediahub",
  "params": {
    ...
    // anniversary/birthday: {"birthday_date": "YYYY-MM-DD", "person_name": "≤6字或null", "background_image": "预设背景ID或null"}
    ...
    // mediahub: {"apps": ["qq_music","cosmos"] 从qq_music/cosmos/bilibili/ximalaya中选1-2个}
  },
  "needs_follow_up": [],
  "ai_generated": {
