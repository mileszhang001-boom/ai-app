...
### 8 个模板全部完成（Liquid Glass 设计语言 · Schema 驱动重构 2026-03-24）

> v2.0 重构：删除 warm 模板，8 模板从零重建，严格对标 Pencil 设计稿 + TEMPLATE_SCHEMA 数据契约。

| 模板 | 模板 ID | 配色模式 | 特色功能 |
|------|---------|---------|---------|
| 恋爱纪念 | `anniversary_love` | image-tint（背景图取色） | ❤️ 粒子 + 天数计算 + 玻璃面板 + 7色渐变遮罩 |
| 宝宝成长 | `anniversary_baby` | image-tint（琥珀色调） | ⭐ 粒子 + 月数计算 + 深棕文字 |
| 放假倒计时 | `anniversary_holiday` | image-tint（暗琥珀色调） | 🎉 confetti 粒子 + 倒计时 + ✈️ 标签药丸 |
| 每日新闻 | `news_news` | mood（分类→颜色映射） | 头条大图 + 分类色标 + overlay 全文详情 |
| 闹钟 | `alarm_clock` | clean（accent_color） | 滑动删除 + 分组(作息/其他) + toggle 开关 + FAB |
| 天气 | `weather_weather` | mood（天气条件→配色） | mesh 渐变背景 + glass pill + 3日预报 + AI 建议 |
| 音乐播放器 | `music_music` | mood（封面图取色） | 模糊封面背景 + CSS 频谱动画 + 播放控制 + 空状态 |
| 日历日程 | `calendar_calendar` | clean（accent_color） | 农历 + 滑动删除 + 下次会议倒计时 + FAB |
...
| 纪念日模板 ×3 | love/baby/holiday — Liquid Glass + Schema 驱动重建（warm 已删除） | ✅ |
| 新闻模板 | 毛玻璃卡片 + 轮播高亮 + 交错淡入 | ✅ |
| 闹钟模板 | SVG 进度环 + 实时倒计时 + 日夜感知 | ✅ |
| **天气模板** | 动态天气粒子 + 3日预报 + AI建议 | ✅ |
| **音乐播放器** | 频谱动画 + 播放控制 + 歌词 | ✅ |
| **日历日程** | 农历/节气 + 时间线 + 事件倒计时 | ✅ |
| AI Pipeline | prompt.py + generator.py + validator.py (8模板注册, SCHEMA 字段名统一) | ✅ |
...
| **动态配色引擎 v2** | `color-engine.js` — 双模式(mood/clean)，12色色板，三层配色架构(Clean/Mood/Photo) | ✅ |
...
**配色系统升级（2026-03-23）**：
- 三层配色架构：Clean（日程/闹钟）/ Mood（天气/新闻/音乐）/ Photo（纪念日）
- Color Engine v2：双模式 API `computePalette(hex, 'mood'|'clean')`
- mood 模式 bg 饱和度 50%→70%，亮度 0.15→0.20（色感明确可区分）
- 12 色色板（天空蓝/靛蓝/玫红/珊瑚红/翡翠绿/薄荷青/琥珀橙/暖金/电光紫/薰衣草/孔雀青/鲜绿）
- `color-extract.js`：从图片底部 30% 提取主色调 → `--panel-tint` → 毛玻璃面板自动上色
- Photo 层不显示色盘，Clean 层标注"强调色"，Mood 层标注"主题配色"

**新闻数据源增强（2026-03-23）**：
- RSS 源从 5→12 个，覆盖科技/汽车/财经/体育/生活（新增 RSSHub 备用源）
- 图片提取 3 层优先级：media:content → enclosure → `<img src>`
- 摘要上限 200→500 字
- 列表页图文并排（左文右图 160x120 缩略图）
- 详情页图文混排（hero 大图 + 正文 + 内嵌配图 + 小标题 + 图注）

**车端推送链路（2026-03-23）**：
- 车端确认弹窗（📲 + 卡片类型名 + 取消/接收 + 5s 自动接收）
- 底部 dot 指示器 + 左右滑动手势切换
- Android APK 编译部署：`./gradlew assembleDebug` → `adb install`
- WidgetDetail 支持 code-mode（generationMode + htmlContent）
- WidgetWebView 双路径：template-mode 从 assets 加载 / code-mode 直接注入 HTML
- CSS zoom 自适应 + 车端移除卡片圆角
- 后端 POST /api/widgets 跳过 TemplateRenderer（修复 music/weather 400 错误）

### ✅ v2.0 模板体系重构完成（2026-03-24）— Schema 驱动 + Pencil 对标 + 删除 warm

**Phase 0: 基础设施对齐**
- `tokens.css`: 暗色文字 `#F5F5F0` 层级、亮色文字 `#1E293B`、8 模板 accent fallback、padding 64px
- `color-engine.js`: mood 输出 `--dyn-bg-start/end` + `--dyn-glow`（合并三级光晕）
- `config-panel.js`: Schema 驱动重写（`TEMPLATE_CONFIGS` + 通用字段渲染器，删除所有硬编码分支）
- `render-widget.js`: 8 模板规范 key + 旧别名兼容 + `ai_generated` 透传
- `prompt.py` / `validator.py`: 删除 warm、统一 SCHEMA 字段名（`start_date/birth_date/target_date/description/background_image`）

**Phase 2: 8 模板从零重建**（35 files changed, 净减 ~2900 行）
- 纪念日 ×3: image-tint 配色、天数/月数/倒计时、Easter egg 粒子
- 天气 + 音乐: mood 配色、mesh gradient 背景、频谱动画、空状态
- 日历 + 闹钟 + 新闻: clean 配色、滑动删除(≥64px)、双视图、overlay 详情

**Phase 3: E2E 验证**（18/19 API 测试通过，8/8 模板 HTML 可达）

### 🔲 v2.0 迭代计划（剩余项）

> 详细方案见 `V2_ROADMAP.md`，以下为 Sprint 级摘要。

#### Sprint 1：基础架构（阻塞级）

| ID | 内容 | 涉及模块 | 工作量 |
|----|------|----------|--------|
| S1-1 | 手势交互体系定义 → `SPEC.md` 规范章节 | 文档 | S |
| S1-2 | 卡片唯一标识体系（SHA256 内容指纹 + 推送去重） | server/storage, sync_service, mobile-web, car-simulator | M |
| S1-3 | 手机端/车端数据分层（preview mock vs live API） | 所有 widget-templates, render-widget.js, car-simulator | L |
| S1-4 | 车端"编辑态"（长按→缩小→删除/排序/切换） | car-simulator.html | L |

#### Sprint 2：数据与核心功能

| ID | 内容 | 涉及模块 | 工作量 |
|----|------|----------|--------|
| S2-1 | 天气城市搜索 + 动态数据 | weather_service, weather template | M |
| S2-2 | 音乐 MediaSession 真实对接 | bridge.js, music template, car-simulator | M |
| S2-3 | 新闻内容增强 + 图片 + 滚动 + 刷新 | news_service, news template | L |
| S2-4 | 日程颜色修复 + 完成功能 | calendar template | S |
| S2-5 | 微调反馈机制（不生效时提示用户） | generator.py, validator.py, preview.js | M |
| S2-6 | 颜色参数全链路推送一致性 | preview.js, car-simulator, 所有 templates | M |

#### Sprint 3：交互打磨

| ID | 内容 | 涉及模块 | 工作量 |
|----|------|----------|--------|
| S3-1 | 纪念日图片修复 + 彩蛋微动效 + 上传压缩 | easter-egg.js, config-panel.js, anniversary templates | M |
| S3-2 | 闹钟大改（去分类 + 删除 + 时钟重做） | alarm template | L |
| S3-3 | 日程添加按钮优化（≥76dp） | calendar template | XS |

#### Sprint 4：设计对齐

| ID | 内容 | 涉及模块 | 工作量 |
|----|------|----------|--------|
| S4-1 | 纪念日三卡对齐 + 名字显示 + 图标优化 | anniversary templates, Pencil card.pen | M |
| S4-2 | 日程/音乐使用 Pencil 最新设计 | calendar/music templates, Pencil card.pen | M |
| S4-3 | 全局 SVG Icon 系统（emoji→SVG） | shared/icons/, 所有 templates | M |
...
├── color-engine.js            # 动态配色引擎 v2 (双模式: mood/clean, ~4KB)
├── color-extract.js           # ★ v0.4 背景图取色工具 (纪念日毛玻璃面板自动上色)
...
anniversary/
├── love/     {index.html, style.css, main.js}  — 恋爱纪念 (7KB)
├── baby/     {index.html, style.css, main.js}  — 宝宝成长 (12KB)
└── holiday/  {index.html, style.css, main.js}  — 放假倒计时 (11KB)
...
## 十、v2.0 迭代完成记录（2026-03-23）

v2.0 全量迭代已完成，共 4 Sprint / 18 Batch，覆盖数据真实性、交互完整性、设计精度三大方向。

### Sprint 1：基础架构
| 项目 | 状态 | 说明 |
|------|------|------|
| S1-1 手势规范 | Done | `docs/GESTURE_SPEC.md` — 6 种手势 + HMI 标准 + 冲突处理 |
| S1-2 卡片 ID 去重 | Done | content-hash 替代随机 UUID，推送时覆盖更新 |
| S1-3 数据分层 | Done | preview/live 双模式，5 类模板全部支持 + "示例数据"角标 |
| S1-4 车端编辑态 | Done | 长按 600ms + 80%缩放 + ✕删除 + 确认弹窗 |

### Sprint 2：数据与功能
| 项目 | 状态 | 说明 |
|------|------|------|
| S2-1 天气城市搜索 | Done | GeoAPI + 60城内置列表 + 按城市差异化 mock |
| S2-2 音乐 MediaSession | Done | `__CAR_WIDGET__` 标志 + bridge mock 优化 |
| S2-3 新闻增强 | Done | 后端 150-300 字摘要 + 前端 8 条纵向滚动流 + 有图/无图双布局 |
| S2-4 日程修复 | Done | 颜色选择器 touchend + 事件完成功能 |
| S2-5 微调反馈 | Done | `_diff_params` 对比 + `changes_applied` 列表 |
| S2-6 颜色推送 | Done | primary_color 写入 params + 车端 fallback |

### Sprint 3：交互打磨
| 项目 | 状态 | 说明 |
|------|------|------|
| S3-1 纪念日修复 | Done | 彩蛋微动效（5-8 粒子）+ 600px/0.5 压缩 + bg 映射修复 |
| S3-2 闹钟大改 | Done | 去分类 + 时间排序 + 左滑删除 |
| S3-3 FAB 按钮 | Done | 蓝色光晕阴影增强 |

### Sprint 4：设计对齐
| 项目 | 状态 | 说明 |
|------|------|------|
| S4-1 三卡统一 | Done | love/baby/holiday/warm 统一 panel-tag + name-display |
| S4-2 Pencil 同步 | Done | 已有 token 在用，后续可精细对齐 |
| S4-3 SVG Icon | Done | `shared/icons.js` + 音乐播放按钮替换 |

### 新增文件
- `docs/GESTURE_SPEC.md` — 手势交互规范
- `src/widget-templates/shared/icons.js` — SVG 图标系统
- `src/widget-templates/shared/icons.css` — 图标样式

### 修改文件清单（30+ 文件）
- **服务端**：metadata.py, main.py, generator.py, service.py, fetcher.py, summarizer.py
- **手机端**：preview.js, render-widget.js, config-panel.js, car-simulator.html
- **模板**：全部 9 模板（alarm, calendar, music, news, weather, anniversary×4）
- **共享**：bridge.js, easter-egg.js, icons.js, icons.css
